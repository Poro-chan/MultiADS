...
def encode_text_with_prompt_ensemble(model, objs, tokenizer, device):
    ...
    anomalous = [
        '{} has a thyroid nodule', '{} shows signs of a thyroid nodule', 'thyroid nodule found on {}',
        '{} with a visible thyroid nodule', 'shows a thyroid nodule in {}', '{} affected by a thyroid nodule'
    ]
    ...
    for obj in objs:
        ...
        for i in range(len(prompt_state)):
            prompted_state = [state.format(ob) for state in prompt_state[i]]
            prompted_sentence = []
