...
class ColonDBDataset(data.Dataset):
	def __init__(self, root, transform, target_transform, aug_rate, mode='test', k_shot=0, save_dir=None, obj_name=None):
		self.root = root
		self.transform = transform
		self.target_transform = target_transform
	...
	def combine_img(self, specie_name):
		specie_name = "anomalous"

		img_paths = os.path.join(self.root, "images")
		files = os.listdir(img_paths)
		img_ls = []
		mask_ls = []
		for i in range(4):
			random_file = random.choice(files)
			img_path = os.path.join(img_paths, random_file)
			mask_path = os.path.join(self.root, "masks", random_file)
			img = Image.open(img_path)
			img_ls.append(img)
...
class ClinicDBDataset(data.Dataset):
	def __init__(self, root, transform, target_transform, aug_rate, mode='test', k_shot=0, save_dir=None, obj_name=None):
		self.root = root
		self.transform = transform
		self.target_transform = target_transform
...
class ISICDataset(data.Dataset):
	...
	def combine_img(self, specie_name):
		...
		img_paths = os.path.join(self.root, "skin", "test", "ISBI2016_ISIC_Part1_Test_Data")
		files = os.listdir(img_paths)
		img_ls = []
		mask_ls = []
		for i in range(4):
			random_file = random.choice(files)
			img_path = os.path.join(img_paths, random_file)
			name,_ = os.path.splitext(random_file)
			mask_name = f"{name}_Segmentation.png"
			mask_path = os.path.join(self.root, "skin", "ISBI2016_ISIC_Part1_Test_GroundTruth", mask_name)
			img = Image.open(img_path)
			img_ls.append(img)
...
class TN3KDataset(data.Dataset):
	...
	def combine_img(self, specie_name):
		...
		img_paths = os.path.join(self.root, "test-image")
		files = os.listdir(img_paths)
		img_ls = []
		mask_ls = []
		for i in range(4):
			random_file = random.choice(files)
			img_path = os.path.join(img_paths, random_file)
			mask_path = os.path.join(self.root, "test-mask", random_file)
			img = Image.open(img_path)
			img_ls.append(img)
...
class HeadCTDataset(data.Dataset):
	def __init__(self, root, transform, target_transform, aug_rate, mode='test', k_shot=0, save_dir=None, obj_name=None):
		self.root = root
		self.transform = transform
		self.target_transform = target_transform
	...
	def combine_img(self, specie_name):
		...
		base_dir = os.path.join(self.root, "brain", "test")

		img_dirs = [
			os.path.join(base_dir, "defect"),
			os.path.join(base_dir, "good")
		]
		...
		img_paths = os.path.join(self.root, "brain", "test")
		files = os.listdir(img_paths)
		img_ls = []
		mask_ls = []
		for i in range(4):
...
class BrainMRIDataset(data.Dataset):
	...
	def combine_img(self, specie_name):
		...
		base_dir = os.path.join(self.root, "brain_tumor_dataset")

		img_dirs = [
			os.path.join(base_dir, "yes"),
...
class Br35HDataset(data.Dataset):
	def __init__(self, root, transform, target_transform, aug_rate, mode='test', k_shot=0, save_dir=None, obj_name=None):
		self.root = root
		self.transform = transform
		self.target_transform = target_transform
	...
	def combine_img(self, specie_name):
		img_dirs = [
			os.path.join(self.root, "yes"),
			os.path.join(self.root, "no")
		]
...
class ChexpertDataset(data.Dataset):
	...
	def combine_img(self, specie_name):
		img_paths = os.path.join(self.root, 'CheXpert-v1.0-small/valid')
		patients = os.listdir(img_paths)
		img_ls = []
		mask_ls = []
		for i in range(4):
			patient = random.choice(patients)
			study_dir = os.path.join(img_paths, patient, "study1")
			random_file = random.choice(os.listdir(study_dir))
			img_path = os.path.join(study_dir, random_file)
			img = Image.open(img_path)
			img_ls.append(img)
			img_mask = Image.fromarray(np.zeros((img.size[0], img.size[1])), mode='L')			
			mask_ls.append(img_mask)
